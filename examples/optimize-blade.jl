...
function turbineopt(cspline, tspline, tsr, pitch, method)
    # ---- DESCRIPTION ----
        # The is the turbine analysis function
        # ---- INPUTS ---- 
        # cspline: chord spline parameters
        # tspline: twist spline parameters
        # tsr: tip-speed ratio
        # pitch: pitch angle at each wind speed
        # method: gradient calculation/implementation method
        # ---- OUTPUTS ----
        # AEP, V, T, P, Q, Np
    ...
    r, chord, theta, Rhub, Rtip = shape(cspline, tspline)   # get blade geometric parameters for aerodynamic analysis
    ...
    tilt = 5.0*pi/180   # tilt angle
    hubHt = 90.0        # hub height
    shearExp = 0.2      # wind shear exponent

    rotorR = Rtip*cos(precone)  # rotor radius
    azimuth = 0.0*pi/180    # azimuth angles at which the blade is o
    rho = 1.225  # air density #*one(pitch[1])

    nV = length(pitch)  # number of wind velocities to test
    Vin = 3.0       # cut-in speed
    Vout = 25.0     # cut-out speed
    ...
    i = Int(round(3/8*length(pitch))) #30  # near rated ############# this needs to be changed????
    ops = windturbine_op.(V[i], Omega[i], zero(rho), r, precone, yaw, tilt, azimuth, hubHt, shearExp, rho)
    outputs = solver.(Ref(rotor), sections, ops)
    Np = outputs.Np

    # define wind speed probability density function (following a weibull distribution)
    k = 2.0  # weibull distribution shape parameter
    Vbar = 6.0  # mean speed of weibull distribution
    A = Vbar / gamma(1.0 + 1.0/k)
    cdf = 1.0 .- exp.(-(V/A).^k)    # cumulative density function

    # integrate the power curve to get AEP
...
function run(method, nP)
    ...
    x0 = [cspline; tspline; tsr; pitch] # starting point
    ...
    function wrappersparse!(y, x)
        ...
        # ---- OUTPUTS ----
        # scaled_AEP: annual energy production of the turbine, scaled down to be value appropriate for optimization 
        ...
        # -> power is below the rated power
    ...
    y = zeros(1 + 2*nP + nP-1 + 17) # objective and contraint values (all in one vector)
    ...
    if method == "sparse"
        ysp = zeros(2*nP + nP-1 + 17)   # objective and contraint values (all in one vector)
    ...
    function optimize(x)
        # ---- DESCRIPTION ----
        # This the main funcrion passed into the optimizer (SNOPT)
        # ---- INPUTS ---- 
        # x: vector of design variable values
        # ---- OUTPUTS ----
        # f: objective value
...
# number of number of wind velocities? pitch angles?
nP = 10
